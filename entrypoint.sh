...
# Older imported eggs may still render an empty IDE cache option, which QEMU
# rejects as an invalid cache value. Normalize only truly empty cache values.
MODIFIED_STARTUP=$(printf '%s' "$MODIFIED_STARTUP" | perl -0pe 's/cache=(?=(?:\s|$|[\")]))/cache=writeback/g')
